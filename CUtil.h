...
#pragma warning(pop)
    ...
    public:
        ...
        // r, g, b: 0 - 255, h: 0 - 360бу, s, v: 0 - 1
        static void RGB2HSV(int nr, int ng, int nb, float& h, float& s, float& v)
        {
            float r = float(nr) / 255.f, g = float(ng) / 255.f, b = float(nb) / 255.f;
        ...
        static void HSV2RGB(float h, float s, float v, int& nr, int& ng, int& nb)
        {
            float hh, p, q, t, ff;
            long i;
            float r, g, b;
        ...
        static void ToHSV(COLORREF clr, float& h, float& s, float& v)
        {
            RGB2HSV(GetRValue(clr), GetGValue(clr), GetBValue(clr), h, s, v);
        }


        static COLORREF FromHSV(float h, float s, float v)
        {
            int r, g, b;
            HSV2RGB(h, s, v, r, g, b);
            return RGB(r, g, b);
